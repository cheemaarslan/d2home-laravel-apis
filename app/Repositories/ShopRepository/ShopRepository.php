...
class ShopRepository extends CoreRepository implements ShopRepoInterface
{
	...
	public function getWith(): array
	{
		...
		$with = [
			...
			'categories.translation' => fn($q) => $q->where(fn($q) => $q->where('locale', $this->language)->orWhere('locale', $locale)),
		];
...
//        }
	 ...
	 */
	private function firstRememberProduct(array $filter, string $locale, ?int $shopId, int $productPerPage): array|Collection
	{
