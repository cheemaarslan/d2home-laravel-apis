...
use App\Http\Requests\Order\UploadPhotoRequest;
...
//            if (count($filter['shop_ids']) === 0) { admin drivers cant show orders
...
//            }
    ...
    public function orderStatusUpdate(int $id, StatusUpdateRequest $request): JsonResponse
    {
        ...
        /** @var Order $order */
        $order = Order::with(['shop.seller', 'deliveryMan', 'user.wallet'])->find($id);
     ...
     * @return JsonResponse
     */
    public function uploadPhoto(int $id, UploadPhotoRequest $request): JsonResponse
    {
        $validated = $request->validated();

        $result = $this->service->uploadPhoto($id, $validated);
    ...
    // Generate dynamic invoices for riders based on a per-km delivery price.
    // public function generateInvoices(): JsonResponse
    // {
    //     $result = $this->service->generateInvoices();
